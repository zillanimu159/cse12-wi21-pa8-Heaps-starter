...
class Maze {
    ...
    /*
     * @param mazeSpec is expected to have all strings with the same length, and to have length at
     * least 1 The strings should contain only the following characters:
     * 
     * - '_': means "empty space" - '#': means "wall" - 'F': means "finish", can only appear once
     * across all strings - 'S': means "start", can only appear once across all strings
     * 
     * Initializes Squares in a contents array such that the start and target fields hold references
     * to the corresponding locations in the contents array
     */
    ...
    /*
     * Produce a grid with # for walls and _ for empty spaces, nothing else (e.g. no start and
     * finish)
     */
    ...
    /*
     * Produce a string array like the arguments to the String[] constructor, but with a '*' in each
     * empty space that is part of the solution.
     * 
     * Assumes that the previous pointers, starting from finish, have been set and following them
     * will reach start.
     */
